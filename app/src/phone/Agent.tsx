              ...
              <div style={{ display: "grid", gap: 2 }}>
                {folders.map((f) => {
                  ...
                  return (
                    <div key={f.name || "root"}>
                      {f.name && (
                        <Row touch onClick={() => setFolded((x) => ({ ...x, [f.name]: !closed }))} aria-expanded={!closed} style={{ gap: 8 }}>
                      ...
                      {!closed &&
                        f.files.map((x) => (
                          <Row key={x.path} touch onClick={() => navigate(file(x.path))} style={{ paddingLeft: f.name ? 28 : undefined, touchAction: "manipulation" }}>
            ...
            {(["self", "other"] as const).map((origin) => {
              ...
              return (
                <section key={origin}>
                  ...
                  <div style={{ display: "grid", gap: 2 }}>
                    {list.map((s) => (
                      <Row key={s.name} touch onClick={() => navigate(file(s.path))} style={{ touchAction: "manipulation" }}>
