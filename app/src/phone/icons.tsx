...
/**
 * The phone's one icon family: outlined, a 24-unit box, one 1.8 stroke, round caps and joins, drawn in
 * currentColor so a control's text colour is its icon colour. Controls use these, never text glyphs or
 * emoji. Loki's own drawings; nothing here is a copied product asset.
 */
const PATHS = {
  home: (
    <>
...
export function Icon({ name, size = 24, title, className, ...rest }: IconProps) {
  return (
    <svg
      ...
      className={className ? `loki-phone-icon ${className}` : "loki-phone-icon"}
