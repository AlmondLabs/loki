...
export function Skills({ api }: { api: GlobalSkillsApi }) {
  ...
  return (
    <div style={{ display: "grid", gap: 10 }}>
      ...
      <div style={{ display: "grid", gap: 2 }}>
        {(skills ?? []).map((g) => (
          <div key={g.name} className="loki-tree-row" style={{ display: "flex", alignItems: "center", gap: 10, padding: "3px 0 3px 8px", borderRadius: 6 }} title={g.description ?? g.path}>
            ...
            <span style={{ fontSize: 10.5, color: "var(--loki-muted)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "45%" }}>{g.source ?? (g.isLink ? "link" : "copy")}</span>
      ...
      {notice && <div style={{ fontSize: 12, color: "var(--loki-muted)" }}>{notice}</div>}
    </div>
  );
}
