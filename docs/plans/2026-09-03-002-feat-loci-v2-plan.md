---
title: Loci v2 — files as desk
type: feat
date: 2026-09-03
supersedes: 2026-09-01-001-feat-loci-plan.md
status: implemented (spike level); demo polish pending
---

# Loci v2 — files as desk
...
- The store/patch protocol and `loci_render`/`loci_author` tools existed so the agent could express
...
- **D3. Files only.** No render tool. `.json` for kit widgets, `.tsx` for custom. Tools shrink to
  `desk_state` and `loci_camera`. The vocabulary moves into `skills/loci/SKILL.md`.
- **D3 note (2026-09-03, first live test).** Ira, whose memory is full of v1, called `loci_render` and Letta closed it as an unknown tool. Decision: no compatibility shims; a fresh conversation (and the skill's explicit "there is no loci_render") is the fix.
...
- **D7. Widget files live outside the repo** (`~/.letta/loci/widgets`). They are user data, they survive
...
- **D9 (2026-09-03, late).** `loci_camera` frames one or many widgets, can dwell before returning, and targets
...
```text
agent ──Write/Edit──▶ ~/.letta/loci/widgets/<desk>/<name>.json|.tsx
                              │ fs.watch + esbuild syntax check        │ Vite HMR
                              ▼                                        ▼
                     mod: manifest, layout, overlay  ◀── WS /loci ──  canvas tab
                              │ turn_start                             gestures, errors
                              ▼
                     <loci-desk> block on the user's next message
```
...
`shared/desk-core.ts`, `app/`, `skills/loci/`.
