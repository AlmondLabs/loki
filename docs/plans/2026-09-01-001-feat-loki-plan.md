...
> Name: **loki** (chosen 2026-09-02). The memory-palace term — the *places* where
...
Build the loki mod and canvas web app in one repo, in two phases: a spike proving the four risky integrations end-to-end (server+page, WS+widget, streamed chat fork, agent tool → canvas), then the demo build-out (widget kit, authoring pipeline, camera, persistence, polish) leading to the recorded clip.
...
**Deferred to follow-up work**: several of the above return if loki graduates from demo to daily driver; `letta mods package` distribution is a natural post-clip beat.
...
- K1. **One repo, thin shim.** All source in `~/Documents/personal/loki` (this repo). A 3-line file in `~/.letta/mods/loki.ts` imports the built mod bundle. Reason: everything in `~/.letta/mods` gets loaded by the harness; a project can't live there.
...
- K8. **State snapshots** debounced (~200ms) to `~/.letta/loki/state/<scope>.json`; loaded on mod activate. Browser reconnect loop with a visible disconnected state.
- K9. **Lean tool surface, four tools**: `loki_render` (instantiate kit widget with data), `loki_author` (write+bundle+load a widget module), `loki_state` (read desk state), `loki_camera` (glide/zoom to widget). Conversation id comes from tool `ctx`, never from the model.
...
- U1 server + `/canvas` + tokenized page. Gotchas found and fixed: eager server start (tabs must survive `/reload`), persistent token at `~/.letta/loki/token`, cache-busting dynamic-import shim (static re-export pinned Node's ESM cache → `/reload` silently ran stale builds), `createRequire` banner (CJS `ws` in an ESM bundle).
...
- U4 `loki_render` + `loki_state` tools — verified.
...
flowchart LR
    ...
    AGENT((agent turn)) -->|loki_render / loki_author / loki_camera| MOD
    MOD <-->|WS: state, modules,\nchat frames, camera| PAGE
    PAGE -->|interactions, chat input| MOD
    FORK -->|streamed reply| MOD
```

Authoring sequence (U6): agent calls `loki_author(id, source)` → mod writes `widgets/<id>.tsx` → esbuild bundles with kit/React external → on success, module pushed over WS, canvas dynamic-imports blob, camera glides (K7); on failure, esbuild/runtime error text returns in the tool result and the agent revises.
...
**Files:** `package.json`, `tsconfig.json`, `src/mod.ts`, `src/server.ts`, `web/index.html`, `web/main.tsx`, `scripts/build.ts`, plus machine-local shim `~/.letta/mods/loki.ts` (install note in README, path not repo-tracked).
...
### U4. `loki_render`: agent lands a widget
...
**Approach:** Register `loki_render` (K9) accepting `{type, title, data, position?}`; validates against registered kit types; writes store; broadcasts. Conversation id from `ctx` (K9).
...
**Goal:** `loki_author` — the demo's signature move.
...
**Approach:** Tool takes `{id, source}`; writes `widgets/<id>.tsx`; esbuild bundles with `external: [react, @loki/kit]` (K4); serves bundle; canvas dynamic-imports and mounts inside an error boundary (AE3). Bundle errors and runtime errors (boundary catch → WS → mod) return to the agent. Host-global mechanism (import map vs window bridge) settled here.
